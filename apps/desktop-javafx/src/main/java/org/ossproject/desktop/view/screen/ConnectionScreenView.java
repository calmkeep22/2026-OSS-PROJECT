...
public final class ConnectionScreenView {
    ...
    public ScrollPane create() {
        ...
        Label description = new Label("App Key와 Secret으로 모의 또는 실전 서버 연결을 준비합니다. "
                + "입력값은 외부로 전송하지 않으며, 저장을 선택하면 현재 Windows 사용자 계정의 DPAPI로 보호합니다.");
        ...
        Button connect = primaryButton("자격증명 확인", () -> {
            char[] secret = appSecret.getText().toCharArray();
            try {
                boolean success = viewModel.testConnection(
                        appKey.getText(), secret, rememberCredentials.isSelected());
                status.accept(success ? "자격증명을 확인했습니다." : viewModel.connectionMessageProperty().get());
            } finally {
        ...
        useStored.setOnAction(event -> {
            boolean success = viewModel.testConnection("", new char[0], false);
            status.accept(success ? "저장된 자격증명을 확인했습니다." : viewModel.connectionMessageProperty().get());
        });
        ...
        FlowPane actions = wrappingRow(8, connect, useStored, deleteStored, issue, revoke);
        ...
        VBox tokenCard = card("연결 정보", informationRow("REST", "데모 · 실제 호출 전"),
                informationRow("WebSocket", "데모 · 연결 전"), informationRow("토큰 만료", tokenExpiry),
