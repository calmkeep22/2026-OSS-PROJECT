...
def _sec_headline(fc: dict) -> str:
    """무엇이 검증됐고 무엇이 안 됐는지를 먼저 말한다."""
    if not fc:
        return ""
    cmp_rows = fc.get("타깃비교", [])
    # 대표 지표는 **날짜 단위로 보정한 값**을 쓴다.
    # 단순 집계는 같은 날 여러 종목이 함께 걸린 걸 독립 관측으로 세어
    # 효과가 부풀려진다. 맨 위에 부풀려진 수를 걸어 두면 안 된다.
    rev = fc.get("날짜단위") or fc.get("급락반등", [])
    best_rev = max(rev, key=lambda r: r.get("순초과", 0)) if rev else None

    vol = next((r for r in cmp_rows if r.get("타깃") == "변동성"), None)
    dr = next((r for r in cmp_rows if r.get("타깃") == "방향"), None)

    kpi = []
    if vol:
        kpi.append((_pct(vol["균형정확도"]), "변동성 균형정확도",
                    "good" if vol.get("유의미") else "bad"))
    if dr:
        kpi.append((_pct(dr["균형정확도"]), "방향 균형정확도",
                    "good" if dr.get("유의미") else "bad"))
    if best_rev:
        kpi.append((f"+{best_rev['순초과'] * 100:.2f}%p",
                    f"급락 후 반등 (|z|>{best_rev['임계값']}, 날짜 보정)",
                    "good"))
    kpi.append((f"{fc.get('종목수', 0)}종목 × {fc.get('평가일수', 0)}일",
                "워크포워드 평가 규모", ""))

    p = ["<h2 id='top'>한눈에 — 무엇이 맞고 무엇이 안 맞는가</h2>",
         _kpi(kpi),
         _table(cmp_rows, "타깃별 측정 결과 (기준선 50%)",
                ["타깃", "모델", "평가건수", "균형정확도", "균형_신뢰하한",
                 "균형_신뢰상한", "유의미", "적중률", "다수클래스비율",
                 "50%초과종목", "종목수"])]

    p.append(
        '<div class="note"><b>왜 적중률이 아니라 균형정확도로 재는가.</b> '
        "단순 적중률은 기준선이 50%가 아니다. 이 구간의 라벨은 한쪽으로 "
        f"{_pct(cmp_rows[0]['다수클래스비율'], 1) if cmp_rows else '약 67%'} "
        "쏠려 있어서, <b>아무 생각 없이 다수쪽만 외쳐도</b> 그만큼 나온다. "
        "55% 짜리 모델에 50% 선을 그으면 넘은 것처럼 보이지만 실제로는 "
        "무지성보다 나쁘다 — 한 번 그렇게 읽고 결론이 뒤집힌 적이 있다.<br><br>"
        "균형정확도는 <b>상승 적중률과 하락 적중률을 따로 재서 평균</b>한다. "
        "한쪽만 외치면 반대쪽이 0%가 되어 정확히 50%로 수렴한다. "
        "쏠림이 얼마든 <b>50%가 진짜 무작위선</b>이므로, 50%를 기준선으로 "
        "두고 비교하는 것이 그대로 정당해진다.</div>")

    p.append(
        '<div class="ok"><b>검증을 통과한 것 둘.</b> '
        "① <b>변동성</b>(내일 크게 움직이는가) — 신뢰구간 하한이 50%를 넘는다. "
        "② <b>급락 후 반등</b> — 임계값을 올릴수록 효과가 커지고, "
        "대조군인 급등 쪽에서는 아무 효과도 없다.<br>"
        "<b>검증을 통과하지 못한 것.</b> <b>방향</b>(내일 오르는가) — "
        "동전 던지기와 구별되지 않는다. 감추지 않고 그대로 싣는다.</div>")
    return "\n".join(p)
...
def _model_by_target() -> str:
    ...
    return (
        ...
        "견주는 표가 나갔던 적이 있다.</p>"
        "<p><b>주력 타깃(변동성)에서 50%를 유의미하게 넘는 모델로 정한다.</b> "
        "방향에서는 어차피 넷 다 미검증이라 우열을 가릴 근거가 없다.</p>"
...
def _pooled() -> str:
    ...
    img = ""
    p = ROOT / "03_forecast" / "figures" / "10_pooled_vs_perstock.png"
    if p.exists():
        img = _img(p, "파운데이션 대 종목별 재학습")
...
def _sec_forecast(r: dict) -> str:
    ...
    p.append('<div class="note"><b>상위 종목은 선별 결과다.</b> '
             "아래 &lsquo;시연 상위&rsquo;는 성적순으로 고른 것이다. "
             "이것만 보고 전체 성능을 판단하면 안 된다 — 전체 분포는 위의 "
             "히스토그램에 있고, 그 그림에는 동전 던지기 분포를 겹쳐 두었다.</div>")
...
def render(res: dict) -> str:
    ...
    body = "\n".join([
        ...
        "python cli.py package        # 파트 README·업로드 목록 갱신</code></pre>",
